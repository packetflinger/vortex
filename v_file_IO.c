...
void VSF_SaveRunes(edict_t *player, char *path)
{
	...
	for (i = 0; i < numRunes; ++i)
	{
		...
		if (index != -1)
		{
			...
			format = strdup(va((char *)SQLITE_INSERTRMETA, 
				index,
			...
			for (j = 0; j < MAX_VRXITEMMODS; ++j)
			{
				format = strdup(va((char *)SQLITE_INSERTRMOD, 
					index,
...
qboolean VSF_SavePlayer(edict_t *player, char *path, qboolean fileexists, char* playername)
{
	...
	{ // real saving
		...
		format = strdup(va((char *)SQLITE_UPDATEUDATA, 
		...
		for (i = 0; i < player->myskills.talents.count; ++i)
		{
			format = strdup(va((char *)SQLITE_INSERTTALENT, player->myskills.talents.talent[i].id,
		...
		for (i = 0; i < numAbilities; ++i)
		{
			...
			if (index != -1)
			{
				format = strdup(va((char *)SQLITE_INSERTABILITY, index, 
		...
		format = strdup(va((char *)SQLITE_UPDATECDATA, 
		...
		format = strdup(va((char *)SQLITE_UPDATESTATS, 
		...
		format = strdup(va((char *)SQLITE_UPDATEPDATA, 
		...
		for (i = 0; i < numWeapons; ++i)
		{
			...
			{
				int j;
				format = strdup(va((char *)SQLITE_INSERTWMETA, 
				 index,
				...
				for (j = 0; j < MAX_WEAPONMODS; ++j)
				{
					format = strdup(va((char *)SQLITE_INSERTWMOD, 
						index,
						j,
		...
		format = strdup(va((char *)SQLITE_UPDATECTFSTATS, 
